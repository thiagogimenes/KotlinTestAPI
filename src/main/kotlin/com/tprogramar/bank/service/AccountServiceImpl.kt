...
class AccountServiceImpl(private val repository: AccountRepository) : AccountService {
    override fun create(account: Account): Account {
       Assert.hasLength(account.name, "[nome] nao pode ser branco!")
       Assert.isTrue(account.name.length >= 5, "[nome] deve ter no minimo 5 caracteres!")

       Assert.hasLength(account.document, "[document] nao pode ser vazio!")
       Assert.isTrue(account.document.length >= 11, "[document] deve ter no minimo 11 caracteres")

       Assert.hasLength(account.phone, "[phone] nao pode ser vazio")
       Assert.isTrue(account.phone.length <= 11, "[phone] telefone invalido.")
    ...
    override fun delete(id: Long) {
        ...
        }.orElseThrow { throw RuntimeException("Id not fond $id") }
    }
}